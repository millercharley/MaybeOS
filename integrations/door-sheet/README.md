...
- **Optional: hold the doors open for guests.** With `HOLD_OPEN_MINUTES` set,
  a member can unlock every door for that long — the same email and code as an
  ordinary entry, recorded in the Access Log under their name, and they can
  lock the doors again early. Two things end the hold: the relay is told to
  switch itself back, and the script schedules a re-lock at the same moment.

  **Check the hardware first.** A door normally gets a one-second pulse. An
  electric strike built for that duty can overheat if it is held energised for
  half an hour; a maglock, or a strike marked *continuous duty*, is made for
  it. If you are not sure, leave this unset.
...
6. **Set the Script Properties.** Project Settings (gear icon) → Script
   ...
   - `HOLD_OPEN_MINUTES` — optional, for example `30`. Lets a member hold every
     door unlocked for that long, for guests arriving. Unset or `0` hides the
     button and refuses the request. **Only switch this on if your door
     hardware is rated to stay unlocked** — see below.
