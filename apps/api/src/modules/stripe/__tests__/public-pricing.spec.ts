...
describe('PublicPricingService', () => {
  ...
  it('returns Stripe’s amounts with the fees billing charges', async () => {
    expect(await service.pricing()).toEqual({
      ...
      plans: [
        { plan: 'FREE', monthlyCents: 0, yearlyCents: 0, perMember: false, transactionFeeCents: 100 },
        { plan: 'PLUS', monthlyCents: 50, yearlyCents: 365, perMember: true, transactionFeeCents: 30 },
        { plan: 'UNLIMITED', monthlyCents: 34900, yearlyCents: 358800, perMember: false, transactionFeeCents: 10 },
      ],
    });
  });
