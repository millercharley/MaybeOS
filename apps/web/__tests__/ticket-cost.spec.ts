import { ticketCost, describeFees, money, PLATFORM_FEE_CENTS } from '@/lib/fees';
...
describe('what a ticket costs the buyer', () => {
  it('adds the FREE-plan fee on top, so a $10 ticket is charged $11.00', () => {
    // The exact figure PAY-04's live test is checking against.
    expect(ticketCost({ ticketCents: 1000, plan: 'FREE' }).totalCents).toBe(1100);
  });
  ...
  it('adds the co-op’s own fee as well, and names it separately', () => {
    ...
    expect(cost.totalCents).toBe(1300);
    expect(describeFees(cost)).toBe('includes $3.00 in fees ($1.00 MaybeOS, $2.00 venue)');
  });
  ...
  it('falls back to the FREE fee rather than charging nothing on an unknown plan', () => {
    ...
    expect(ticketCost({ ticketCents: 1000, plan: 'MYSTERY' }).platformFeeCents).toBe(PLATFORM_FEE_CENTS.FREE);
  });
