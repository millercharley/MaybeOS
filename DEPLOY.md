...
| `GOOGLE_SERVICE_ACCOUNT_JSON` | optional; lets MaybeOS write a co-op's door-code sheet (DOR-01). The JSON key of a Google Cloud **service account**, raw or base64 — base64 is safer here, because a multi-line private key pasted into an environment variable reliably loses its newlines. Each co-op then shares its sheet with that account's `…iam.gserviceaccount.com` address as an Editor; Settings shows the address to share with. Unset, door access cannot be switched on |
