...
| _(door access, DOR-01)_ | no environment variable. Each co-op's door sheet is written by an Apps Script in the co-op's own Google account (`integrations/door-sheet/`). The web app address and a signing secret are set per co-op in Settings → Door access; the secret is sealed with secret-box. `GOOGLE_SERVICE_ACCOUNT_JSON` is no longer read |
